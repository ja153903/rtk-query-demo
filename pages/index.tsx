...
const Home: NextPage = () => {
  ...
  if (isLoading) {
    return (
      <div className="container p-5">Loading data...</div>
    );
  }

  if (error) {
    return (
      <div className="container p-5">Something went wrong..</div>
    );
  }
    ...
    <div className="container p-5">
      ...
      <ul>
        {data?.map((promotion: Promotion) => (
          <li key={promotion.id}>{promotion.promotion_code} - {promotion.description}</li>
        )) ?? null}
      </ul>
    </div>
  );
